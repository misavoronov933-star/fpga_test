...
![Проверка AXI Boundary](img/axi_boundary_check.png) <center>Рис.1. Проверка AXI 4K Boundary


![Проверка Циклического буфера с разбиением одной транзакции записи данных](img/ram_256Mb_check.png) <center>Рис.2. Проверка Циклического буфера с разбиением одной транзакции записи данных


![Проверка Циклического буфера с двумя транзакциями записи данных](img/ram_236Mb_check_2.png) <center>Рис. 3. Проверка Циклического буфера с двумя транзакциями записи данных

### Примечания

Порт `addr\_debug` может использоваться в тестбенче для проверки поведения модуля около конца циклического буфера. В рабочей конфигурации начальный адрес равен нулю.

!\[Проверка 4 KB boundary](img/axi\_boundary\_check.png)

!\[Проверка Циклического буфера с разбиением одной транзакции записи данных](/img/ram\_256Mb\_check.png)

!\[Проверка Циклического буфера с двумя транзакциями записи данных](/img/ram\_256Mb\_check\_2.png)
