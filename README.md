...
2\. При поступлении `pattern\_write = 1` модуль сохраняет `pattern\_data` и `pattern\_size`.
...
2\. Убедиться, что файл `axi\_writer.sv` добавлен в Design Sources.

3\. Убедиться, что файл `tb\_axi\_writer.sv` добавлен в Simulation Sources.
